 ...
 * Local functions.
 */
void	fwrite_obj_room	args( ( ROOM_INDEX_DATA *ch,  OBJ_DATA  *obj, FILE *fp, int iNest ) );
void	load_room_obj args( ( ROOM_INDEX_DATA *room , FILE *fp) );
...
void load_room_obj(ROOM_INDEX_DATA *room, FILE *fp)
{
	...
	for ( ; ; )
	{
	    ...
	    word = fread_word( fp );
	    if ( !str_cmp( word, "Object" ) ) fread_obj_room  ( room, fp );
	    else bug("load_room_obj: invalid keywowrd.", 0);
	}
}
