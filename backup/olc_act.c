    ...
    {  
        ...
        {  
            ...
            {  
                found = TRUE;  
                sprintf( buf, "[%5d] %s",
                    pMobIndex->vnum, total_length( pMobIndex->short_descr, 17 ) );
                add_buf( buf1, buf );
		add_buf( buf1, "{x" );
                if ( ++col % 3 == 0 )  
                    add_buf( buf1, "\n\r" );  
...
{  
    ...
    sprintf( buf, "Clan:        [%s{x - %d]\n\r",
        clan_table[pObj->clan].who_name, pObj->clan);
    send_to_char( buf, ch );
...
/*    sprintf( buf, "Wealth:      [%5ld]\n\r",  
    ...
    sprintf( buf, "Clan:        [%s - %d]\n\r",
        clan_table[pMob->clan].who_name, pMob->clan);
    send_to_char( buf, ch );
...
{  
    ...
    if ( !IS_BUILDER( ch, pArea ) )  
    {  
        send_to_char( "MEdit: That vnum is in an area you don't have access to.\n\r", ch );
        return FALSE;  
    }  
...
{  
    ...
    {  
        ...
        send_to_char( "In order to see the list of damage messages, type '? weapon'.\n\r", ch );  
        return FALSE;  
    }  
...
{  
    ...
    {  
        ...
        if ( !pMob->pShop )  
        {  
            send_to_char( "MEdit:  You must create a shop first (shop assign).\n\r", ch );  
            return FALSE;  
        }  
...
{  
        ...
        {  
                send_to_char( "Syntax: sector [type] [optional lower] [optional upper]\n\r", ch );  
                return FALSE;  
        }  
        if (vnlower < 0 || vnupper < vnlower)
        {
	        send_to_char( "Those vnums are not valid, try again!\n\r",ch);
	        send_to_char( "Make sure vnums are greater than 0 and the min is lower than the max.\n\r",ch);
	        return FALSE;
        }
    	...
    	{
	    	if ((get_room_index(vnlower)->area == get_room_index(start)->area) && (get_room_index(vnupper)->area == get_room_index(start)->area))
	    	{
			...
			{
				send_to_char("You can only use this function on rooms in the area you are in.\n\r",ch);
				return FALSE;
			}
		}
		else
