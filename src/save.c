...
/* New DB style */
void    save_chararacter_full args( ( CHAR_DATA *ch ) );
// -- Saves Following:
//    - PLAYER
//    - PSTATIC
//    - POBJECTS
void    save_character_plr  args( ( CHAR_DATA *ch,  FILE *fp ) );
void    save_character_sav  args( ( CHAR_DATA *ch,  FILE *fp ) );
void    save_character_obj  args( ( CHAR_DATA *ch,  OBJ_DATA  *obj, FILE *fp, int iNest ) );

void    load_chararacter_full args( ( DESCRIPTOR_DATA *d, char *name ) );
void    read_character_plr  args( ( CHAR_DATA *ch,  FILE *fp ) );
void    read_character_sav  args( ( CHAR_DATA *ch,  FILE *fp ) );
void    read_character_obj  args( ( CHAR_DATA *ch,  FILE *fp ) );
...
void    fread_char_reroll args( ( CHAR_DATA *ch,  FILE *fp ) );
...
/*
 * Tell save_character_specs to write player information.
 */
void save_character_full( CHAR_DATA *ch )
{
    save_char_obj(ch);
    return;
}
...
/*
 * Write the non-tier saving PLAYER data.
 */
void save_character_plr( CHAR_DATA *ch, FILE *fp )
{
	...
	for ( sn = 0; sn < MAX_SKILL; sn++ )
	{
            if ( skill_table[sn].name != NULL && ch->pcdata->learnlvl[sn] > -1 )
	    {
                fprintf( fp, "Sklvl %d '%s'\n",
                    ch->pcdata->learnlvl[sn], skill_table[sn].name );
	    }
	}
...
/*
 * Write the cross-tier saving PLAYER data.
 */
void save_character_sav( CHAR_DATA *ch, FILE *fp )
{
/*    AFFECT_DATA *paf; */
    int /*sn,*/ pos;
/*    int ut; */

    fprintf( fp, "#%s\n", IS_NPC(ch) ? "MOB" : "PSTATIC" );
    ...
    if (ch->description[0] != '\0')
    	fprintf( fp, "Desc %s~\n",	ch->description	);
    if (ch->prompt != NULL || !str_cmp(ch->prompt,"<%g/%Hhp %n/%Mmp %w/%Vmv> "))
        fprintf( fp, "Prom %s~\n",      ch->prompt  	);
    if (ch->clan) {
...
/*
 * Write the OBJECT data carried by PLAYER/PSTATIC
 */
//bree here too ios
void save_character_obj( CHAR_DATA *ch, OBJ_DATA *obj, FILE *fp, int iNest )
{
    EXTRA_DESCR_DATA *ed;
    AFFECT_DATA *paf;
    ...
    if ( obj->next_content != NULL)
        save_character_obj( ch, obj->next_content, fp, iNest);

    /*
     * Castrate storage characters.
    if ( (ch->level < obj->level - 5 && obj->level > 19
    ...
    /* KEYS WILL DISAPPEAR ON LOG OUT NOW AGAIN by Dusk */
...
void fwrite_char( CHAR_DATA *ch, FILE *fp )
{
    ...
    if (ch->petition)
	fprintf( fp, "Petit %s~\n",	clan_table[ch->petition].name);    fprintf( fp, "Deity %s~\n",(god_table[ch->god].name));
    if (ch->color != 0)
	fprintf( fp, "Color %d\n", ch->color);
    if (ch->color_auc != 0)
	fprintf( fp, "Coauc %d\n", ch->color_auc);
...
/*        if (ch->pcdata->colour_name == '\0')
	...
	fprintf( fp, "Cnd  %d %d %d %d\n",
        ...
        if (ch->nextquest != 0)
            ...
            fprintf( fp, "QuestNext %d\n", ch->countdown   );
        if(ch->questmob > 0 && ch->questgiver != NULL)
         fprintf( fp, "QuestMob %d %d\n", ch->questmob, ch->questgiver->pIndexData->vnum);
        if(ch->questobj > 0 && ch->questgiver != NULL)
         fprintf( fp, "QuestObj %d %d\n", ch->questobj, ch->questgiver->pIndexData->vnum);
        if(ch->questobj > 0 && ch->questroom > 0 && ch->questgiver != NULL)
         fprintf( fp, "QuestRoom %d\n", ch->questroom);
 ...
 * Load a character
 */
bool load_character_full( DESCRIPTOR_DATA *d, char *name )
{
    char strsave[MAX_INPUT_LENGTH];
    char buf[100];
    CHAR_DATA *ch;
    ...
    ch->comm				= COMM_COMBINE 
					| COMM_PROMPT
					| COMM_STORE;
    ch->prompt 				= str_dup("<%g/%Hhp %n/%Mmp %w/%Vmv> ");
    ...
    /* player files parsed directories by Yaz 4th Realm */
#if !defined( macintosh )
    sprintf( strsave, "%s%s%s%s%s", PLAYER_DIR, initial( name ),
            "/", capitalize( name ), ".gz" );
#else
    ...
    /* player files parsed directories by Yaz 4th Realm */
#if !defined( macintosh )
    sprintf( strsave, "%s%s%s%s", PLAYER_DIR, initial( name ),
            "/", capitalize( name ) );
#else
...
/*    ch->pcdata->colour_name             = str_dup( "" ); */
    ...
    if(found && ch->questobj > 0)
    {
        room = get_room_index(ch->questroom);
        questobj = create_object( get_obj_index(ch->questobj), ch->level );
...
/*    ch->pcdata->colour_name             = str_dup( "" ); */
	...
	for ( ; ; )
	{
	    ...
	    word = fread_word( fp );
            if      ( !str_cmp( word, "PLAYER" ) ) fread_char_reroll( ch, fp );
	    else if ( !str_cmp( word, "END"    ) ) break;
	    else
	    {
                bug( "Load_char_reroll: bad section.", 0 );
		break;
	    }
	}
	fclose( fp );
...
void fread_char( CHAR_DATA *ch, FILE *fp )
{
	...
	case 'A':
	    ...
	    KEY( "Arank",	ch->pcdata->arank,	fread_number( fp ) );
	...
	case 'C':
         ...
         KEY( "Cln",    ch->clan, clan_lookup(fread_string(fp)));
          KEY( "ClanRank", ch->pcdata->clan_rank,       fread_number( fp ) );
...
/*            KEY( "ClrName",     ch->pcdata->colour_name,fread_string( fp ) ); */
	...
	case 'N':
	    KEY( "Name",	ch->name,		fread_string( fp ) );
            KEY( "NCln",        ch->cln,                fread_string( fp ) );
	...
	case 'P':
	    ...
	    KEY( "Prank",	ch->pcdata->prank,	fread_number( fp ) );
	...
	case 'Q':
            ...
            if (!str_cmp(word,"QuestMob"))
	    {
                ...
                ch->questmob = fread_number( fp );
                vnum = fread_number( fp );
                ch->questgiver = (CHAR_DATA *)get_mob_index( vnum );
		fMatch = TRUE;
		break;
	   }
		break;
	...
	if ( !fMatch )
	{
            ...
            sprintf( lbuf, "Fread_char: no match on word '%s'.", word );
            bug( lbuf, 0 );
	    fread_to_eol( fp );
	}
    }
