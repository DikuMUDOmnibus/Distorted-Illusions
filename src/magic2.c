...
void spell_conjure( int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
	...
	if (stone->value[0] < 1)
	{
     ...
     if(ch->class == CLASS_VOODAN || ch->class == CLASS_FORSAKEN
      || ch->class == CLASS_WIZARD || ch->class == CLASS_MAGE || ch->class == CLASS_ARCHMAGE)//breehere
      {
	       CHAR_DATA *pet;
...
void spell_enforcer(int sn,int level, CHAR_DATA *ch,void *vo,int target)
{
        ...
        for ( i = 0; i < 2; i++)
        {
         ...
         guard->level = number_range(ch->level,ch->level*4/2);
...
void spell_overgrowth( int sn, int level, CHAR_DATA *ch, void *vo,int target)
    ...
    if (IS_SET(ch->in_room->affected_by,ROOM_AFF_OVERGROWN) || ch->in_room->sector_type == SECT_FOREST)
    {
    ...
    act( "$n begins to chant as vines and trees burst into existence throughout the room!\n\r", ch, NULL, NULL, TO_ROOM );
    send_to_char( "Vines and trees burst into existence throughout the room!\n\r", ch );
    return;
}
