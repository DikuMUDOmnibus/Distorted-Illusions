...
bool saves_spell_real( int level, CHAR_DATA *victim, int dam_type, bool show,CHAR_DATA *ch )
{
    ...
    // Returned to 0 to 65 by Fesdor to decrease general effectiveness.
    saves = URANGE(0, saves, 65); 
...
void spell_siphon_life( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    */
    ...
    fbonus = 0;
    dbonus = 0;
	mod = number_range(get_curr_stat(ch,STAT_WIS)*4,get_curr_stat(ch,STAT_WIS)*5)-(ch->saving_throw - victim->saving_throw);
...
void spell_demonfire(int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    if (check_sorcery(ch,sn))
        dam = sorcery_dam(level*4.5,10,ch);
    else 
    	dam = dice( level*4.5, 10 );
    ...
    {
       if (check_sorcery(ch,sn))
        dam = sorcery_dam(level*6,10,ch);
       else 
        dam = dice( level*5.5, 10 );
    }
...
void spell_mana_blast (int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    dam = dice( level*5, (ch->mana*10.5/ch->max_mana)*8/4 ); //minor damage tweak 4/12/09 - bree 10 to 10.5 *8/4 from *8/5
...
void spell_fork(int sn,int level,CHAR_DATA *ch,void *vo,int target)
{
    ...
    act("$n releases a bolt of fire from their hand which Forks and strikes $N!",ch,NULL,victim,TO_NOTVICT);
    act("$n releases a bolt of fire from their hand!",ch,NULL,victim,TO_VICT);
    act("You send a bolt of fire at $N and it forks into two!",ch,NULL,victim,TO_CHAR);
    ...
    dam = dice(ch->level/15,(dam*3)/2.3);
